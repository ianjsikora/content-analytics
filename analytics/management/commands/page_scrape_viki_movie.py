...
def page_scrape(args):
    ...
    for i in range(1,1058):

        #Get IMDb ID from Viki Title
        viki_qs = Viki.objects.filter(id=i)
        url="http://www.imdb.com/find?ref_=nv_sr_fn&q=" + str(viki_qs[0]) + "&s=movies#tt"
        print url
        r = requests.get(url)
        ...
        try:
            ...
            try:
                ...
                viki_link_qs = Viki.objects.get_or_create(id=i)
                content_qs[0].viki = viki_link_qs[0]
